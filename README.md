...
| Datei | Worum es geht |
|---|---|
| [`docs/Das-ganze-Werkzeug.pdf`](docs/Das-ganze-Werkzeug.pdf) | **Zum Kontrollieren, nicht zum Nachschlagen.** Seite 1–2: 26 offene Fragen an den Betrieb. Danach, was über Ablauf, Oberfläche und Rechnung angenommen wird — jede Annahme einzeln zum Abhaken, jede Frage des Betriebs gegen die Zahl gehalten, die sie beantwortet, die Kaskade an einem von Hand nachgerechneten Beispiel, und am Schluss zehn belegte Fehler. 22 Seiten. |
...
| **Ursachen → „Palox: Faules im Lager": die x-Achse reicht bis −1000 Tage, alle Punkte sitzen als Strich rechts** | Eine Palette wurde beim Zählen mit einem Datum in der Zukunft erfasst (etwa Jahr 2029 statt 2026). Die Auswertung machte daraus negative Lagertage und hielt den Punkt für plausibel; das Diagramm gab ihm die ganze Achse. Beides ist in Runde N repariert (`docs/GEGENPROBE_BEFUND.md`, N-01 und N-02): mit Schema 70 sind negative Lagertage nie plausibel und die Achse folgt keinem Ausreisser mehr — siehst du es noch, läuft deine Datenbank auf einem älteren Stand (Schritt 3 nochmal ausführen) | Messungen → Auffälligkeiten: die Zeile „Zetteldatum … an dem Tag kam keine Palette dieser Charge" nennt die Arbeit. Dort das Datum der Palette korrigieren, dann „Neu rechnen". Die Beobachtung bleibt bis dahin so gespeichert, wie sie getippt wurde — das ist richtig so |
...
Was in den Warenausgangsdateien steht, gemessen an den echten Dateien:

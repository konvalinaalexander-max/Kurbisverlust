...
| **„numeric field overflow"** (im Überblick oder beim Ausführen von `setup.sql`) | Eine Formel ist an einer Messung gesprengt worden — am 7. September an Wägungen, bei denen die Palette schwerer war als beim Eingang. Seit 0056 zählt so eine Wägung nicht mehr, und `setup.sql` rechnet nur noch am Ende mit den heutigen Formeln | Schritt 3 nochmal ausführen. Bleibt die Meldung: `supabase/diagnose.sql` in den SQL-Editor einfügen und ausführen — die Tabelle unten sagt, welche Sicht scheitert und was in den Rohdaten auffällig ist; Ergebnis wörtlich weitergeben. |

-- sicht: v_koeff_verdunstung_geschaetzt
-- Der Verdunstungs-Koeffizient je Sorte, gepoolt: eigener Mittelwert und Gesamtmittel, gewichtet nach Streuung (tau2, b). Sorten mit wenigen Messungen rücken damit näher an den Gesamtwert, statt an einem Ausreisser zu hängen.

 WITH roh AS MATERIALIZED (
         SELECT v_koeff_roh_verdunstung.art,
            v_koeff_roh_verdunstung.sorte,
            v_koeff_roh_verdunstung.charge_nr,
            v_koeff_roh_verdunstung.anteil,
            v_koeff_roh_verdunstung.gewicht
           FROM v_koeff_roh_verdunstung
          WHERE v_koeff_roh_verdunstung.anteil IS NOT NULL AND v_koeff_roh_verdunstung.gewicht > 0::numeric
        ), je_charge AS (
         SELECT roh.art,
            roh.sorte,
            roh.charge_nr,

...
-- =====================================================================
-- aus 0070_die_zeit_laeuft_vorwaerts.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0072_die_erfassung_scharf_schalten.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0073_palox_je_arbeit_und_ehrliches_alter.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0075_woher_die_verderbszahl_kommt.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0078_das_lager_nach_kaliber_und_fax_auf_eis.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0079_der_messtag_der_nenner_und_die_glocke.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0080_tabula_rasa_vor_dem_testtag.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0081_die_demo_zeigt_was_die_app_kann.sql
-- =====================================================================

-- =====================================================================
-- 0081 — Die Demo-Saison zeigt, was die App heute kann
--
-- Die Saison aus 0052 war für den Stand von damals gebaut. Seither hat das
-- Werkzeug zehn Dinge dazugelernt, und keines davon war in der Demo zu
-- sehen. Nachgemessen auf einer frisch geladenen Demo-Datenbank (alle 31
-- Tabellen, alle 119 Sichten):
--
--   * drei Sichten blieben leer — v_auftrag_fertige_masse (fertige
--     Paletten), v_kontrollpalette_rate (Lagerkontrolle) und
--     v_ausgang_pruef (Datei gegen Lieferung);
--   * zwei Tabellen blieben leer — kontrollpalette und
--     kontrollpalette_wiegung, also der ganze Bildschirm „Palette
--     kontrollieren", und ausgang_datei, also der Warenausgang-Import;
--   * fünfzehn Spalten wurden nie gefüllt: fertige_paletten_gesamt,
--     geplante_paletten, durchsatz_kg, kaliber_von_g, kaliber_bis_g,
--     auftrag_palette.palette_id und .gebindeart, verdunstung_wiegung
--     .palette_id, .faul_kg und .auswahl, charge.ernte_abgeschlossen_ts,
--     sortier_lauf.roh_datei_ref, auftrag_teilnehmer.verlassen_ts,
--     ausgang_zeile.datei_id und .erloes.
--
-- Wer die Demo anschaute, sah also ein Werkzeug, das weniger kann als das
-- echte. Das ist der schlechteste Fehler, den eine Demo machen kann.
--
-- Diese Saison füllt alles davon — und sie ist eine andere: andere Mengen,
-- andere Erntewochen, andere Anteile, alle 42 Chargen der Anbauplanung
-- statt 36 (sechs standen vorher ohne eine einzige Zeile da), und erfundene
-- Abnehmer statt der echten Namen. Eine Demo, die jeder anschauen darf,
-- nennt keine Kunden.
--
-- Was neu dazukommt, und warum es dazugehört:
--
--   Kontrollpaletten. Je Sorte eine gekennzeichnete Palette, die über Wochen
--   immer wieder auf dieselbe Waage kommt (bis zu zwölf Wägungen). Zwei
--   Wägungen derselben Palette ergeben eine Verdunstungsrate ohne jede
--   Annahme — der sauberste Wert, den der Betrieb hat. Eine zeigt am Ende
--   sichtbaren Schimmel (dann taugt das Paar nicht mehr), eine ist
--   inzwischen verarbeitet und beendet.
--
--   Fertige Paletten. Zwei von drei Arbeiten zählen am Ende, wie viele
--   fertige Paletten dastanden. Ohne diese Zahl kennt die App nur die
--   *gewogene* Palette und die Ausgangsmasse bleibt unbekannt — nicht null.
--   Die dritte zählt sie nicht, denn so ist es im Betrieb auch.
--
--   Die Verkaufsdatei als Datei. Die Zeilen der Warenwirtschaft kommen als
--   Excel-Datei, einmal im Monat, mit Prüfsumme und Zeitraum — und mit
--   Erlös je Position. Dazu zwei Abweichungen zwischen Datei und Lieferung,
--   damit der Abgleich nicht leer dasteht.
--
--   Die gegriffene Eingangspalette. Zwei von drei Arbeiten greifen die
--   Palette aus der Liste (dann ist ihre Masse exakt), die dritte tippt nur
--   das Zetteldatum ab (dann ist sie ein Mittel). Die Herkunftsspalte im
--   Dashboard zeigt beides.
--
--   Eigenes Kaliber, Durchsatz statt Paletten, Schichtwechsel, wie die
--   Palette gegriffen wurde, wie viel davon faul war, welche Chargen fertig
--   geerntet sind — alles Fälle, die die Masken kennen und die Demo bisher
--   nicht hatte.
--
-- Unverändert bleibt, was die Demo tragbar macht: alles ist deterministisch
-- (kein random(), zweimal laden gibt zweimal dieselbe Saison), alles ist
-- markiert (Arbeiten „DEMO", Paletten „demo-…", Dateien „DEMO-…"), alles
-- lässt sich mit demo_daten_entfernen() restlos wieder entfernen, und im
-- Echtmodus lässt sich das hier gar nicht erst laden (0072).
-- =====================================================================
...
begin
  ...
  return (
    select format('Demo-Saison steht: %s Paletten in %s Chargen, %s Arbeiten (davon %s Fax), %s Sortierläufe, '
                  || '%s Lieferungen, %s Kontrollpaletten mit %s Wägungen, %s Verkaufsdateien. '
...
comment on function demo_daten_laden is
  'Legt die erfundene Demo-Saison an (Arbeiten mit bemerkung = ''DEMO'', Paletten mit extern_id ''demo-…'', Sortierdateien ''DEMO-…'', Lieferungen und Verkaufsdateien ''DEMO'', Kontrollpaletten mit bemerkung ''DEMO''). Echte Daten bleiben unberührt; im Echtmodus lässt sie sich gar nicht laden (0072). Deterministisch: zweimal laden gibt zweimal dieselbe Saison (0081).';
grant execute on function demo_daten_laden() to authenticated;


-- ---------------------------------------------------------------------
-- 2. Entfernen: was 0081 zusätzlich anlegt, muss auch wieder weggehen
-- ---------------------------------------------------------------------
-- Neu gegenüber 0052: die Kontrollpaletten samt ihren Wägungen, die
-- Verkaufsdateien und der Erntestand der Chargen. Der Erntestand steht an
-- einer Stammdatenzeile, die die Demo nicht angelegt hat — deshalb wird er
-- nur dort zurückgesetzt, wo Demo-Paletten hängen, und zwar bevor die
-- Paletten verschwinden. Sonst wüsste danach niemand mehr, welche Chargen
-- gemeint waren.
...
begin
  ...
  -- Zuerst der Erntestand: er wird an den Demo-Paletten erkannt.
  update charge set ernte_abgeschlossen_ts = null
   where ernte_abgeschlossen_ts is not null
     and nr in (select distinct charge_nr from palette where extern_id like 'demo-%');

  -- Die Kontrollpaletten vor den Paletten — sie zeigen mit palette_id darauf.
  delete from kontrollpalette_wiegung
   where kontrollpalette_id in (select id from kontrollpalette where bemerkung = 'DEMO');
  delete from kontrollpalette where bemerkung = 'DEMO';
  ...
  delete from ausgang_datei where quelle = 'DEMO' and bemerkung = 'DEMO';
  ...
  return (
    select format('Demo-Daten entfernt. Übrig: %s Paletten, %s Arbeiten, %s Sortierläufe, %s Kontrollpaletten.',
                  (select count(*) from palette),
                  (select count(*) from auftrag),
                  (select count(*) from sortier_lauf),
                  (select count(*) from kontrollpalette))
  );
end $fn$;
...
comment on function demo_daten_entfernen is
  'Löscht restlos alles, was demo_daten_laden() angelegt hat — samt Kontrollpaletten, Verkaufsdateien und dem Erntestand, den die Demo an den Chargen gesetzt hat. Echte Daten bleiben unberührt; erkannt wird die Demo an bemerkung = ''DEMO'', extern_id ''demo-…'' und datei_name ''DEMO-…'' (0081).';
grant execute on function demo_daten_entfernen() to authenticated;


-- ---------------------------------------------------------------------
-- 3. Wer die Demo betritt, sieht auch das Büro
-- ---------------------------------------------------------------------
-- In der Demo soll ein Besucher beides sehen: die Halle und das Dashboard.
-- Anmelden kann er sich dort nur anonym — kein Konto, kein Passwort, ein
-- Name und los. Anonyme Anmeldungen werden aber zu Arbeitern, und damit
-- bliebe ihm ausgerechnet die Hälfte verschlossen, die erklärt, wozu das
-- Werkzeug überhaupt da ist.
--
-- Die Regel hängt deshalb an der Datenbank, nicht an der App: Sagt die
-- Datenbank von sich selbst, dass sie eine Beispieldatenbank ist
-- (einstellung.betriebsmodus = 'beispiel'), wird jedes neu angelegte Konto
-- zum Betriebsleiter. In der Datenbank des Betriebs steht 'echt' — dort
-- ändert sich kein Komma, und ein Arbeiter bleibt ein Arbeiter. Dieselbe
-- Überlegung wie in 0072: Eine Datenbank weiss immer, was sie ist, und kann
-- nicht behaupten, die andere zu sein. Die App ist hier nur die Tür, nicht
-- das Schloss.
--
-- Unverändert bleibt alles andere aus 0035: anonyme Nutzer werden als solche
-- markiert, ein leerer Name zählt wie keiner, und „Gast" ist die letzte
-- Rückfallebene, damit die Anmeldung nie an einem fehlenden Namen scheitert.
create or replace function public.handle_new_user()
returns trigger language plpgsql security definer set search_path = public as $$
declare
  v_beispiel boolean := coalesce(
    (select wert #>> '{}' from einstellung where schluessel = 'betriebsmodus'), 'echt') = 'beispiel';
begin
  insert into public.profil (id, name, anonym, rolle)
  values (
    new.id,
    coalesce(nullif(new.raw_user_meta_data->>'name', ''),
             ...
             'Gast'),
    new.email is null,
    case when v_beispiel then 'admin' else 'arbeiter' end::rolle
  )
  on conflict (id) do nothing;
  return new;
end $$;
...
comment on function public.handle_new_user is
  'Legt zu jedem neuen Konto das Profil an. In einer Beispieldatenbank '
  '(einstellung.betriebsmodus = ''beispiel'') wird es zum Betriebsleiter, damit '
  'ein Demo-Besucher auch das Dashboard sieht; in der Datenbank des Betriebs '
...
-- =====================================================================
-- aus 0084_der_palox_wird_mittendrin_geleert.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0085_die_rueckmeldung_am_ende_jeder_arbeit.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0088_der_ausschuss_wird_kiste_fuer_kiste_gewogen.sql
-- =====================================================================
...
-- =====================================================================
-- aus 0089_eine_verdunstung_die_keine_ist.sql
-- =====================================================================
...
-- einmal — in 121 Schritten, in der Reihenfolge, in der eine auf der
...
-- =====================================================================
-- aus 0087_die_auffaelligkeit_ausschuss_tara_kennt_die_palette.sql
-- =====================================================================

-- =====================================================================
-- 0087 — Die Auffälligkeit „Ausschuss-Tara" kennt die Palette
--
-- Gefunden von der Kette (pruefstand/kette_pruefen.sh) beim Rückwärts-
-- Einspielen der Runde V: Die Kiste Ausschuss von 12 kg ohne Palette,
-- seit 0083 richtig als 11 kg gespeichert, stand sofort als Auffälligkeit
-- da — „aus Brutto 12.00 kg und heutiger Tara wären es 0 kg".
--
-- Die Nachrechnung in v_plausibilitaet (0066/0070) war die alte Rechnung:
-- immer die Palette abziehen, und ein negatives Ergebnis mit greatest(…, 0)
-- auf null klemmen. Genau die zwei Fehler, die 0083 im Auslöser beseitigt
-- hat, lebten hier ein drittes Mal weiter. Sie hätten jede richtige
-- Ausschuss-Zeile ohne Palette als Tippfehler gemeldet.
--
-- Jetzt rechnet die Probe so wie der Auslöser: die Palette nur, wenn sie
-- mitgewogen wurde (mit_palette), und ohne Klemme — ein negatives Netto
-- kann seit 0083 nicht mehr gespeichert werden; steht eines aus alter Zeit
-- da, soll die Probe es zeigen, nicht verstecken.
--
-- Die Sicht wird als Ganzes neu gestellt, wie in 0066 und 0070; geändert
-- ist allein der Zweig „Ausschuss-Tara".
-- =====================================================================
 ...
 SELECT 'Ausschuss-Tara'::text AS art,
   ...
   FROM ((((ausschuss_messung m
     ...
     CROSS JOIN LATERAL ( SELECT ((m.brutto_kg - ((m.kisten)::numeric * g.tara_kg_pro_kiste)) -
        CASE WHEN m.mit_palette THEN g.tara_kg_palette ELSE (0)::numeric END) AS roh) n)
...
comment on view v_plausibilitaet is
  'Messungen, die die Auswertung bewusst nicht verwendet — und Messungen, die '
  'sie nicht verwenden kann, weil ihnen der Nenner fehlt. Neu (0051): '
  'Fax-Anteile, Zetteldaten ohne Palette, Kisten nach Sollgewicht ohne gewogene '
  'Palette. 0066: „Ausschuss-Tara" rechnet nur nach, wo Kistenzahl und '
  'hinterlegte Tara da sind; fehlt eine, steht die Lücke als „Ausschuss ohne '
  'Tara" daneben. 0087: die Nachrechnung zieht die Palette nur ab, wenn sie '
  'mitgewogen wurde, und klemmt nichts auf null.';
grant select on v_plausibilitaet to authenticated;
comment on view v_verdunstung_messung is

...
echo "Simulation: $N Saisons · ${LAGER} im Lager · Selektion=$SELEKTION · $WIEGUNGEN Wiegungen · $KONTROLLEN Lagerkontrollen"
...
for i in $(seq 1 "$N"); do
  psql "$URL" -v ON_ERROR_STOP=1 -q -v lauf="$i" -v selektion="$SELEKTION" \
       ...
       -v n_lagerkontrollen="$KONTROLLEN" -f "$HIER/saison.sql" >/dev/null
...
with wahr as (
  select lauf, groesse, rank() over (partition by lauf order by wert desc) as rang
    from sim.wahrheit
   where groesse in ('Verdunstung', 'Schimmel/Fäulnis', 'Ausschuss zu klein')
), geschaetzt as (
...
select s.groesse,
       count(*)                                                    as laeufe,
       round(avg((s.mittel - w.wert) / w.wert * 100), 1) || ' %'   as verzerrung,
       round(stddev_samp((s.mittel - w.wert) / w.wert * 100), 1) || ' %' as streuung,
